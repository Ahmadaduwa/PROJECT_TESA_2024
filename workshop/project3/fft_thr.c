...
void *fft_thr_fcn(void *ptr) {
    ...
    while(1) {
        ...
        // Convert indices to Hz based on sampling rate (assume 48kHz sampling rate)
        ...
        for (int j = 0; j < 3; j++) {
            ...
            printf("Frequency %d: %f Hz, Amplitude: %f\n", j + 1, frequency, top_amplitudes[j]);
        }

        // Classify sound as bass or treble based on max frequency amplitude
        if (top_indices[0] * freq_resolution < 300.0) {
            printf("This is a bass sound\n"); //(ทุ้ม)
        } else if (top_indices[0] * freq_resolution > 1000.0) {
            printf("This is a treble sound\n"); //(แหลม)
        }
